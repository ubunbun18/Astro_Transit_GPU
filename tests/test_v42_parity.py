...
def test_v42_exact_parity():
    np.random.seed(42)
    time = np.linspace(0, 27, 2000).astype(np.float64)
    flux = np.random.normal(0, 0.003, size=2000).astype(np.float64)
    ...
    assert correlation > 0.99999, f"Correlation failed: {correlation}"
    
    # Check max difference
    max_diff = np.max(np.abs(cpu_power - gpu_power))
    assert max_diff < 1e-4, f"Max difference is too large: {max_diff}"
